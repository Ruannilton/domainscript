...
func emitHTTPHelpers(e *emit.Emitter, runtimeAlias string) {
	...
	e.Line("// runtime.ErrForbidden (403) e runtime.ErrNotFound (404); qualquer outro")
	e.Line("// error (infra) -> 503. Rate-limit (429), tenant ausente (400) e sunset")
	e.Line("// (410) são marcos posteriores (G4/G5/G6) — deliberadamente não tratados")
	e.Line("// aqui, para não travar a extensão futura com suposições erradas.")
