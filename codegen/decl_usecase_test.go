...
func emitWalletUseCases(t *testing.T) []byte {
	...
	got, err := codegen.EmitUseCases("wallet", decls, aggregates, prog, model, prog.Symbols, "Wallet", reg, nil)
	if err != nil {
...
func TestEmitUseCaseGoldenSingle(t *testing.T) {
	...
	got, err := codegen.EmitUseCase("wallet", decls[0], aggregates, prog, model, prog.Symbols, "Wallet", reg, nil)
	if err != nil {
...
func meterSmokeFiles(t *testing.T) map[string][]byte {
	...
	ucGo, err := codegen.EmitUseCase("meter", meterUseCaseDecl(), aggregates, prog, model, prog.Symbols, "Meter", reg, nil)
	if err != nil {
...
func TestEmitUseCaseUnknownAggregateFallsThroughToBuiltinError(t *testing.T) {
	...
	if _, err := codegen.EmitUseCase("wallet", uc, map[string]*ast.AggregateDecl{}, prog, model, prog.Symbols, "Wallet", goname.NewVOOperatorRegistry(), nil); err == nil {
