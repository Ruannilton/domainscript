...
- DESCRIPTION: [`design.md`](../specs/correcoes-issues-6-8-12/design.md) §5.1
  ("Como `aggregateType` chega a `Append` (decisão de M1.1)") registra a
  decisão do usuário — carimbar `ctx` UMA vez, em
  [`codegen/decl_usecase.go`](../../codegen/decl_usecase.go), imediatamente
  antes de `uow.Run(ctx, ...)` — mas condiciona explicitamente essa rota a
  uma premissa: "Isso só é seguro se uma única `Tx.Run()` **nunca** grava
  eventos de mais de um `aggregateType`... **M1.1 deve confirmar essa
  premissa por leitura**... Se a premissa **não** se confirmar (uma `Run()`
  mistura tipos), essa rota não serve — M1.1 para e reporta, não adivinha um
  fallback." A própria task
  ([`tasks/M1.1.md`](../specs/correcoes-issues-6-8-12/tasks/M1.1.md), banner
  "Desbloqueada" + Step 2.2) repete a mesma condição.
  ...
  **2. A própria spec da linguagem exemplifica "mesmo Database → commit
  ...
  `.claude/steerings/domainscript-spec-v7/19-transactions-sagas.md:7`: "Mesmo
