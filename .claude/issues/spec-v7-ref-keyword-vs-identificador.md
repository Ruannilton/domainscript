...
- TASK: [review-v7.md §A-6](../steerings/review-v7.md)
- DESCRIPTION: A
  [§5.1](../steerings/domainscript-spec-v7/05-application-layer.md)
  (Commands) exige `ref` como **keyword** na declaração de campo —
  `walletId ref Wallet` — e a implementação a leva como hard keyword
  (`token.REF`). A [§2.5](../steerings/domainscript-spec-v7/02-type-system.md)
  (tipo `File`), no mesmo documento, usa `ref` como
  ...
  **A spec precisa decidir**: (a) manter `ref` reservado e reescrever o
  ...
  precedente disso
  (`nameableKeywords`, [`parser/parse_decl.go:98-111`](../../parser/parse_decl.go#L98-L111)),
  o que torna (c) a saída mais barata sem perder nenhuma das duas formas.
