...
- DESCRIPTION: Tudo está atrás de seams limpos (NFR-12 respeitado), mas a única
  ...
  **não é implantável contra infraestrutura real** além disso. Categorias em
  ...
  decorativos — [`codegen/sql_wiring.go`](../../codegen/sql_wiring.go));
  ...
  in-memory — [`codegen/channel_test.go`](../../codegen/channel_test.go));
  ...
  [`.claude/specs/infra-providers/`](../specs/infra-providers/requirements.md)
  (Marco J, REQ-41..48 / NFR-21..24) tratou esta issue com **recorte de 5
  providers** — Postgres, RabbitMQ, Redis (Cache+RateLimit), S3 e Outbox
  durável. As demais categorias de G-4 (outros bancos, gRPC-canal, Dynamo
  para idempotency `external`, backend `layered` de cache, GCS/Azure) ficam
  explicitamente fora do recorte, para ciclos futuros.
  ...
  `sql_wiring.go` — não há pacote `pgrt` separado, citado por engano ao
  registrar esta issue), RabbitMQ (J3,
  [`codegen/channel_rabbitmq.go`](../../codegen/channel_rabbitmq.go)), Redis
  ...
  determinism_test.go`). Ver [`.claude/specs/codegen/gaps.md`](../specs/codegen/gaps.md)
  §G-4 para a tabela completa antes/depois por categoria.
  ...
  ([`.claude/specs/correcoes-issues-9-10-11/`](../specs/correcoes-issues-9-10-11/requirements.md),
