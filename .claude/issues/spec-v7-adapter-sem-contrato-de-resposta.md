...
- DESCRIPTION: `result = call PaymentRequest(...)` aparece em dois lugares da
  spec —
  [`19-transactions-sagas.md`](../steerings/domainscript-spec-v7/19-transactions-sagas.md)
  §19.2 (o exemplo canônico de Saga) e
  [`09-notifications-adapters.md`](../steerings/domainscript-spec-v7/09-notifications-adapters.md)
  §9.2 — e `mock PaymentRequest returns PaymentResult(status:
  PaymentStatus.Declined)` aparece em
  [`24-testing.md`](../steerings/domainscript-spec-v7/24-testing.md) §24.3.
  Em nenhum dos dois casos a spec declara o **tipo** do valor
  atribuído/mockado:
  ...
  - `Adapter` Nível 2 — FFI vinculado (§9.3) referencia `function
    ...
    `Foreign` genérico
    ([`10-ffi.md`](../steerings/domainscript-spec-v7/10-ffi.md) §10.2) tem
  ...
  - `PaymentResult`, o tipo usado no `mock ... returns PaymentResult(...)` do
    ...
    spec — `grep -rn "PaymentResult" .claude/steerings/domainscript-spec-v7/`
  ...
  **Evidência de código** (a implementação reflete fielmente essa lacuna, não
  ...
  <Notif>) error` ([`codegen/decl_io.go`](../../codegen/decl_io.go)) — sem
  canal de valor de retorno.
  [`codegen/lower/builtins.go:340`](../../codegen/lower/builtins.go#L340)
  recusa `result = call Adapter(...)` explicitamente:
  ```go
  case "call":
  ...
  ```
  E [`codegen/gentest.go`](../../codegen/gentest.go#L1330) (`emitSagaMock`)
  hoje descarta o valor de `mock ... returns X` (`_ = <expr>`) — o mock
  nunca influencia o fluxo do passo seguinte.
  ...
  **O que a spec precisa decidir** (decisão de linguagem, não de
  ...
  [`correcoes-issues-6-8-12/design.md`](../specs/correcoes-issues-6-8-12/design.md)
