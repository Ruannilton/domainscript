...
- DESCRIPTION: A [§14](../steerings/domainscript-spec-v7/14-multi-tenancy.md)
  ...
  **Impacto.** A §14 exige que o opt-in cross-tenant venha acompanhado de role
  ...
  `sema/rules_crossfile.go`) e o warning de auditoria também
  (`checkCrossTenantAudit`), mas **a sintaxe que expressaria a exigência de
  role não existe** — hoje só dá para declarar o opt-in, não a proteção que
  a spec pede junto.
