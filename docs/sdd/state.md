...
`docs/sdd/specs/<spec>/state.md` (ou `tasks.md` no modelo legado) e em
`docs/sdd/issues/open-issues.md`. Ver `CLAUDE.md`.

## Próxima spec-task

`docs/sdd/specs/correcoes-issues-6-8-12/tasks/M4.2.md` → **M4.2** — Staging na
...
`docs/sdd/issues/spec-v7-adapter-sem-contrato-de-resposta.md`. M2.3 (`emit` em
...
Issue: `docs/sdd/issues/m2-3-mecanismo-de-emit-em-passo-de-saga-exige-arquivos-fora-de-target-files.md`;
...
`docs/sdd/issues/m1-4-produtor-duravel-query-le-store-em-memoria-nao-o-banco-real.md`,
achado independente de REQ-55.7/55.8 durante M1.4.

## Próxima issue

`docs/sdd/issues/visibility-de-view-nao-implementado.md` — sem spec dedicada
