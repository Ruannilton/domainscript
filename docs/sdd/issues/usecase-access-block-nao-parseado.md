...
- SPEC: transpilador
- TASK: descoberto ao escrever os exemplos spec-first (review-v7.md §A-9)
- DESCRIPTION: A §14 (Multi-Tenancy) fecha com o exemplo canônico de acesso
  ...
  **Escopo a confirmar ao implementar.** A spec mostra esta forma **uma única
  vez**, de passagem, no exemplo da §14; a §5.2 (UseCases) não menciona
  `access`. Então dá para implementar o que está escrito (`access { requires
